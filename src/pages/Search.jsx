...
const GENRES = [
  { label: 'Hip-Hop', gradient: 'linear-gradient(135deg,#0b7a5c,#173664)' },
  { label: 'Électro', gradient: 'linear-gradient(135deg,#0e926f,#10203c)' },
  { label: 'Jazz', gradient: 'linear-gradient(135deg,#13a37e,#17355f)' },
  { label: 'Pop FR', gradient: 'linear-gradient(135deg,#2ab08f,#27507d)' },
  { label: 'R&B', gradient: 'linear-gradient(135deg,#107d67,#183968)' },
  { label: 'Classique', gradient: 'linear-gradient(135deg,#0f9b77,#1b4f7e)' },
  { label: 'Ambient', gradient: 'linear-gradient(135deg,#1f8c76,#1a2747)' },
  { label: 'Rock', gradient: 'linear-gradient(135deg,#0a7f61,#122640)' },
  { label: 'Soul', gradient: 'linear-gradient(135deg,#1ba88a,#19355d)' },
]
...
export default function Search() {
  ...
  async function loadMore() {
    ...
    try {
      ...
      setApiError(error.message || 'Impossible de charger plus de résultats.')
    } finally {
  ...
  async function importCurrentQuery() {
    ...
    try {
      ...
      setImportMessage(`${data.imported} nouveaux titres importés dans la DB locale (${data.total} au total).`)
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div>
          ...
          <p className={styles.sub}>Catalogue local persisté + recherche YouTube live. La base locale reste disponible même sans connexion utilisateur.</p>
        </div>
        {query.trim() && <a className={styles.youtubeBtn} href={youtubeSearchUrl(query)} target="_blank" rel="noreferrer">Voir plus sur YouTube ↗</a>}
      </div>

      <div className={styles.quickStats}>
        <div className={styles.quickStat}><span>Catalogue local</span><strong>{catalogStats.total || TRACKS.length}</strong></div>
        <div className={styles.quickStat}><span>Chaînes indexées</span><strong>{catalogStats.channels || 0}</strong></div>
        <div className={styles.quickStat}><span>Source live</span><strong>{catalogStats.youtubeConfigured ? 'YouTube API' : 'Démo locale'}</strong></div>
      </div>
      ...
      {!catalogStats.youtubeConfigured ? <div className={styles.notice}>Ajoute <code>YOUTUBE_API_KEY</code> dans le backend et redémarre le serveur pour activer la recherche/import YouTube.</div> : null}
      {apiError ? <div className={styles.notice}>{apiError}</div> : null}
      {importMessage ? <div className={styles.notice}>{importMessage}</div> : null}

      {isAdmin && query.trim() ? <div className={styles.adminRow}><button className={styles.importBtn} onClick={importCurrentQuery} disabled={importing || !catalogStats.youtubeConfigured}>{importing ? 'Import…' : 'Importer 3 pages dans la DB locale'}</button><span className={styles.adminHint}>L’import reste réservé à l’admin.</span></div> : null}

      {query.length > 1 ? (
        <section>
          <div className={styles.sectionTitleRow}><div className={styles.sectionTitle}>{loading || catalogLoading ? 'Recherche…' : results.length > 0 ? `${results.length} résultat${results.length > 1 ? 's' : ''}` : 'Aucun résultat'}</div><span className={styles.resultMeta}>{catalogTotal} titres en DB locale</span></div>
          <div className={styles.list}>{results.map((t, idx) => <TrackRow key={`${t.id}-${idx}`} track={t} index={idx + 1} />)}</div>
          {activeCat !== 'Catalogue local' && remoteResults.length > 0 && nextPageToken ? <div className={styles.actionsRow}><button className={styles.loadMoreBtn} onClick={loadMore} disabled={loadingMore}>{loadingMore ? 'Chargement…' : 'Charger plus de résultats live'}</button></div> : null}
          {!loading && !catalogLoading && results.length === 0 && <div className={styles.empty}><span>🔍</span><p>Aucun titre trouvé pour « {query} »</p><span className={styles.emptySub}>Essaie un autre mot-clé, ou importe des pages dans la DB locale pour enrichir ton catalogue.</span></div>}
        </section>
      ) : (
        <>
          <section>
          ...
          <section>
            <div className={styles.sectionTitle}>Catalogue déjà disponible</div>
