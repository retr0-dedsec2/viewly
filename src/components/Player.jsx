...
export default function Player({ isOpen = true, onToggleOpen = () => {} }) {
 const {
 currentTrack,
 isPlaying,
 progress,
 ...
 return (
 <footer className={`${styles.player} ${isOpen ? '' : styles.collapsed}`}>
 ...
 {!isOpen ? (
 <>
 <button className={styles.miniTrack} onClick={onToggleOpen} aria-label="Ouvrir le lecteur">
 <div className={styles.miniArt} style={{ backgroundImage: `url(${currentTrack.thumbnail})` }} />
 <div className={styles.miniInfo}>
 <strong>{currentTrack.title}</strong>
 <span>{currentTrack.artist}</span>
 <div className={styles.miniProgress} aria-hidden>
 ...
 <button className={styles.expandBtn} onClick={onToggleOpen} aria-label="Ouvrir le lecteur">
 <ChevronUp size={17} />
 </button>
 </div>
 </>
 ...
 <button className={`${styles.ctrl} ${shuffle ? styles.ctrlActive : ''}`} onClick={toggleShuffle} aria-label="Aleatoire" aria-pressed={shuffle}>
 <Shuffle size={16} />
 </button>
 <button className={styles.ctrl} onClick={skipPrev} aria-label="Precedent">
 <SkipBack size={17} />
 </button>
 <button
 ...
 <button className={`${styles.ctrl} ${repeatMode !== 'off' ? styles.ctrlActive : ''}`} onClick={toggleRepeat} aria-label={repeatMode === 'one' ? 'Repeter un titre' : 'Repeter la file'} aria-pressed={repeatMode !== 'off'}>
 ...
 <button className={styles.ctrl} onClick={() => setLibraryOpen(true)} aria-label="Ouvrir la bibliotheque">
 <LibraryIcon size={16} />
 </button>
 ...
 <button className={styles.ctrl} onClick={onToggleOpen} aria-label="Fermer le lecteur">
 <ChevronDown size={16} />
 </button>
